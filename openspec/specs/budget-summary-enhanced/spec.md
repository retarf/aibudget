# budget-summary-enhanced Specification

## Purpose

The budget summary endpoint reports both aggregate and per-category planned
vs actual figures. Replaces the requirements previously defined in
`transaction-summary-api`: the gateway now aggregates two NATS calls —
planned amounts from budget-service and actual sums from transaction-service
